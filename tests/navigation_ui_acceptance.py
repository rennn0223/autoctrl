...
child=pexpect.spawn('scripts/start-exhibition',['--skip-robot','--skip-bridges','--','--ros-args','-p','doctor_timeout_s:=0.2'],encoding='utf-8',timeout=30,env={**os.environ,'TERM':'xterm-256color'},dimensions=(45,150))
...
try:
 ...
 for name,text,match,timeout in [('waypoints','先到（0.7, 0），再到（1.4, 0.3）','導航完成',70),('figure8','走八字，半徑 0.8 公尺','導航完成',240),('vision','看看前面','正在看前方畫面',15)]:
  ...
  if name=='vision':
   child.expect('›');child.sendline('走八字');child.expect('可取消。');child.expect('›');child.sendline('停止');child.expect('已停止並取消導航')
